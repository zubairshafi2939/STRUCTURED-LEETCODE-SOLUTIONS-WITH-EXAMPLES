...
class Solution(object):
    def delNodes(self, root, to_delete):
        ...
        def order(root,res,data):
            if root:
                if root.val in data:
                    if root.left and root.left.val not in data:
                        res.append(root.left)
                    if root.right and root.right.val not in data:
                        res.append(root.right)
                    root.left = order(root.left,res,data)
                    root.right = order(root.right,res,data)
                    return None  
                root.left = order(root.left,res,data)
                root.right = order(root.right,res,data)
        order(root,res,data)       
